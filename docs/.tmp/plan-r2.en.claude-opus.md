# Integrated Remediation Plan (R2): P1 and P2 Gap Resolution
...
Per U-P2-1 through U-P2-7:

(a) Add definitions entries for all currently undefined trigger
    identifiers, using hyphen-separated lowercase keys.
(b) Normalize all condition identifier tokens across the file to
    hyphen-separated lowercase form matching definitions keys.
(c) Preserve the " and " separator in compound conditions per the
    existing compound_conditions specification.
...
P2-B requires updating conditions values across 44 rules. All four
source plans agree this normalization is necessary (U-P2-2, U-P2-3).
Although the number of affected rules is large, the operation is
strictly mechanical: a fixed 7-entry mapping table (section 3.3) is
applied to all conditions arrays. No judgment or semantic analysis is
required for the replacement itself, and the operation does not alter
any rule's applicability (U-P2-7).

This mechanical nature means the per-rule risk is low despite the high
count. The alternative of adding definitions with space-separated quoted
keys to avoid normalizing conditions was analyzed (copilot plan,
Approach 2) and rejected by all four plans because: (a) YAML keys with
spaces require quoting, which is unconventional; (b) it would leave the
file with two incompatible identifier conventions; (c) scope identifiers
would still need normalization regardless.

### 3.6 Convention Statement (SHOULD -- per M-1)

A convention statement SHOULD be added to the interpretation section
under a new key (e.g., condition_identifiers) declaring:

- Condition identifiers in conditions arrays MUST use hyphen-separated
  lowercase form matching the corresponding definitions key.
- In compound conditions of the form "A and B", both A and B MUST be
  hyphen-separated identifiers defined in definitions.

This is a single new key under interpretation, not a new rule record.
It prevents future regressions where new identifiers are added in a
non-canonical form.

| ID   | Target                 | Type    | Note                 |
| ---- | ---------------------- | ------- | -------------------- |
| P2-C | interpretation section | Add key | Convention statement |
...
| Phase 2 | P2-A, P2-B, P2-C | Resolve condition identifier vocabulary |
...
(b) For each condition entry:
    - If simple (no " and " separator): confirm it exists as a key in
      definitions.
    - If compound ("A and B"): split on " and ", confirm both A and B
