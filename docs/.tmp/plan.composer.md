...
**パス補足**: `.agents/skills/` が実体であり、`.*/skills/`（例: `.cursor/skills/`）はそこを指すシンボリックリンクである。
...
| ---------------------------------- | ----------------------- | -------------------------------------------------------------------------------- | ------------------------ |
| `explanatory-must-not-for-clarity` | `authoring_obligations` | 例示・列挙として登場。主述語は SHOULD。                                          | **非違反**（説明的使用） |
...
### 3.5 追加の観点（Composer 分析からの補足）

- **prohibitions の位置**：現状は YAML ブロックの中ほど（interpretation, precedence, failure_states, definitions の後）に配置されている。lost-in-the-middle 対策を最大化するなら、`interpretation` の直後に移動することが望ましい。
- **definitions.tier-separation**：`definitions.tier-separation.description` の "MUST NOT interleave" は、tier-separation 適用時には実質的に規範的禁止として機能する。`explanatory-must-not-permitted` が definitions を許容範囲に含むかはスキル内で明示されていない。将来的に `no-interleave-tiers` を prohibitions に追加し、definitions から参照する形に整理する選択肢がある。

---

## 4. 現考案の課題（Composer による指摘）

現考案（ユーザー提示の草案）に対する Composer の評価で特定された課題を以下に列挙する。

### 4.1 新規ルール A-1 のスキーマ不足

`no-treat-explanatory-must-not-as-prohibition` の案に、以下が欠けている：

- `exceptions`
- `verification`

`rule-record-schema` と `verification-criterion-per-must` により、全ルールにこれらが必要。

**補完案**：

```yaml
exceptions: ["none"]
verification: "No interpretation, semantics, or verification field treats descriptive MUST NOT as an enforceable prohibition; human or pattern check."
```

### 4.2 prohibitions セクションの位置に関する扱い

- **現考案**：Section 8 で「未解決・検討継続事項」として先送り。
- **Composer 分析**：Section 1 で lost-in-the-middle を重視しているなら、prohibitions の位置変更は「別タスク」ではなく、本タスクの一部として扱うか、少なくとも「次フェーズで実施する」と明記すべき。

**推奨**：本プランに「Phase 2：prohibitions の序盤移動」を組み込み、実施順序を明確化する。

### 4.3 definitions.tier-separation の扱い

方針ドキュメントでは `definitions.tier-separation.description` の "MUST NOT interleave" を説明的使用として許容している。

一方で、tier-separation が適用される文脈では実質的に規範的禁止として機能する。`explanatory-must-not-permitted` の「interpretation, semantics, or verification」に definitions が含まれるかはスキル内で明示されていない。

**推奨**：現状は許容として進めるが、`explanatory-must-not-permitted` の conditions または definitions に「definitions フィールドを含む」旨を明示する。将来的に `no-interleave-tiers` を prohibitions に追加する選択肢を残す。

### 4.4 prohibitions.override の適用範囲

現考案 Section 8 で「確認が必要」とされたが、Composer 評価では問題なしと判断。`prohibitions.override` は format/content/authoring_obligations を上書きするため、新規 prohibition（A-1）は authoring_obligations の A-2 を上書きしうる。両者は補完関係にあり矛盾しない。**確認不要**としてよい。
...
**分割後ルール A-1（`prohibitions` セクションへ追加）**
...
#### 変更 C：`prohibitions` セクション内の順序調整

移動した `no-treat-explanatory-must-not-as-prohibition` を、
`prohibitions.items` リストの末尾（既存 8 項目の後）に追加する。

#### 変更 D（Phase 2・任意）：`prohibitions` セクションの序盤移動

lost-in-the-middle 対策を最大化するため、YAML ブロック内で `prohibitions` を
`interpretation` および `precedence_and_conflict` の直後に移動する。

- 新ルール `prohibitions-section-early` の追加を検討：「prohibitions セクションは interpretation および precedence_and_conflict の直後に配置する MUST」。
- 実施は Phase 1（meta-circular 回復）完了後、別タスクまたは Phase 2 として扱う。
...
| D: prohibitions 序盤移動           | YAML ブロック全体                                      | Phase 2 で検討               |

---

## 7. 適用強度についての最終方針

### 7.1 「どこまでの強さで適用するか」への回答
...
| Tier                         | 内容                                                                                                                               |
| ---------------------------- | ---------------------------------------------------------------------------------------------------------------------------------- |
| **Tier 0（構造確認）**       | SKILL.md の全 rule record を列挙し、`statement` フィールドで MUST NOT が主述語である全ルールを確定リスト化する。                   |
| **Tier 1（変更実施）**       | 変更 A（A-1 新規追加 + A-2 statement 書き換え）を実施する。変更 C（prohibitions 末尾への追加）を含む。                             |
| **Tier 2（整合性検証）**     | YAML パース検証、構造検証、meta-circular 確認（`prohibitions-dedicated-section` 遵守）、`one-obligation-per-rule` 遵守を確認する。 |
| **Tier 3（主観的品質確認）** | 変更後の記述が意図を保持しているか、人間レビューまたは LLM レビューで確認する。                                                    |
| **Phase 2（任意）**          | 変更 D（prohibitions 序盤移動）を実施する。                                                                                        |
...
1. **`prohibitions` セクションの位置**：Phase 2 として `interpretation` の直後への移動を検討する。大きな構造変更となるため、Phase 1 完了後に実施する。

2. **他の AI directive files への展開**：本リポジトリにスキルファイルが複数存在する場合、同様の分析を全ファイルに適用するか、authoring スキルの meta-circular 遵守回復を優先するか、方針を確定する必要がある。

3. **`explanatory-must-not-permitted` の許容範囲の明示化**：definitions フィールドを許容範囲に含む旨を、条件または definitions に明示することを検討する。

4. **`definitions.tier-separation` の `no-interleave-tiers` 化**：将来的に `no-interleave-tiers` を prohibitions に追加し、definitions から参照する形に整理する選択肢を残す。
